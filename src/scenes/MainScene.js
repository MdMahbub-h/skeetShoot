...
export class MainScene extends Scene {
    ...
    initGameState() {
        ...
        this.targetTypes = [
            { key: "bottle", points: 100, baseScale: 0.9 },
            { key: "can", points: 150, baseScale: 0.9 },
            { key: "dynamite", points: 200, baseScale: 0.7 },
            { key: "hat", points: 300, baseScale: 0.5 },
        ];
    }
    ...
    spawnTarget() {
        ...
        if (!this.isMobile) {
            target.setVelocity(velocity.x * 1.2, velocity.y * 1.2);
            target.body.gravity.y = 700 * this.scaleFactor;
        } else {
            target.setVelocity(velocity.x / 1.5, velocity.y * 1.35);
            target.body.gravity.y = 1800 * this.scaleFactor;
        }
    ...
    showGameOver() {
        ...
        restartButton2.setOrigin(0.5);
        restartButton2.setDepth(5);
        restartButton2.setInteractive({ useHandCursor: true });
        if (win) {
            restartButton2.setText("Play Again");
        }
        // Hover effects
        restartButton2.on("pointerover", () => {
            restartButton2.setStyle({ fill: "#ffff00" });
        });

        restartButton2.on("pointerout", () => {
            restartButton2.setStyle({ fill: "#fff" });
        });
        ...
        if (win) {
            restartButton.setText("Try Medium");
        } else {
            restartButton2.destroy();
        }
    }
    ...
    setupMobileControls() {
        ...
        this.shootButton = this.add.image(
            ...
            gameHeight - this.getScaledSize(130),
            "shoot-button"
        );
